...
async function analyzePsd(psdPath, workDir) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (!label) {
      backgroundLayers.push({ path: layerPngPath, offsetX: geo.offsetX, offsetY: geo.offsetY });
      continue;
    }
    ...
    fields.push({
      name: label,
      type,
      x: (absX + localBbox.w / 2) / canvasWidth,
      y: (absY + localBbox.h / 2) / canvasHeight,
      w: localBbox.w / canvasWidth,
      h: localBbox.h / canvasHeight,
  ...
  // lequel chaque calque de fond est composité à sa position réelle — jamais
  // l'inverse (un calque recadré plus petit utilisé comme base), qui provoquait
  // l'erreur "Image to composite must have same dimensions or smaller".
  let composite = sharp({ create: { width: canvasWidth, height: canvasHeight, channels: 4, background: { r: 255, g: 255, b: 255, alpha: 1 } } });
  if (backgroundLayers.length > 0) {
    composite = composite.composite(backgroundLayers.map(l => ({ input: l.path, left: Math.max(0, l.offsetX), top: Math.max(0, l.offsetY) })));
  }
