...
function countLayers(psdPath) {
  const out = execFileSync('identify', [psdPath]).toString().trim();
...
function extractLayerToPng(psdPath, index, outPath) {
  execFileSync('convert', [`${psdPath}[${index}]`, '-alpha', 'on', `PNG32:${outPath}`], { stdio: ['ignore', 'ignore', 'ignore'] });
}
...
async function analyzePsd(psdPath, workDir) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // mais on le garde dans le fond (il fait partie du décor).
