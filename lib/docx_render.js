...
async function renderPng(templatePath, fields, values, options = {}) {
  ...
  for (const f of imageFields) {
    ...
    if (!buf) continue;
    let w = Math.max(1, Math.round(f.w * W));
    let h = Math.max(1, Math.round(f.h * H));
    if (f.shape === 'circle') w = h = Math.min(w, h); // un cercle reste un cercle
    const resized = await sharp(buf).resize(w, h, { fit: 'cover' }).png().toBuffer();
    const shaped = await clipToShape(resized, w, h, f.shape);
    composites.push({
      input: shaped,
