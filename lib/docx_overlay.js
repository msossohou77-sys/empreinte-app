...
async function overlayPositionedFields(pdfBuffer, positionedFields, values) {
  ...
  for (const f of positionedFields) {
    ...
    const wPt = f.w * pageWidth;
    const hPt = f.h * pageHeight;
    ...
    if (f.type === 'image') {
      ...
      if (parsed) {
        embedded = parsed.mime.includes('png') ? await pdfDoc.embedPng(parsed.buffer) : await pdfDoc.embedJpg(parsed.buffer);
      }
